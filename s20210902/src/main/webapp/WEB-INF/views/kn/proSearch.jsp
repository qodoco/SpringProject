...
function wishInsert(pcode){
	$.ajax({
		...
		success : function(data){
			if(data > 0){
				var result;
				result = confirm("위시리스트에 등록 되었습니다.\n위시리스트로 이동하시겠습니까?");
				if(result == true){
	    	...
	    	<div class="col-lg-12">
	    		<div class="product__pagination">       	
					<c:if test="${pg.startPage > pg.pageBlock }">
						<a href="proSearch?currentPage=${pg.startPage-pg.pageBlock}&sort=${sort}">[이전]</a>
					</c:if>
					<c:forEach var="i" begin="${pg.startPage}" end="${pg.endPage}">
						<a href="proSearch?currentPage=${i}&sort=${sort}" class="<c:if test="${pg.currentPage == i }">active</c:if>">${i}</a>
					</c:forEach>
					<c:if test="${pg.endPage < pg.totalPage }">
						<a href="proSearch?currentPage=${pg.startPage+pg.pageBlock}&sort=${sort}">[다음]</a>
					</c:if>
				</div>
			</div>
		</div>			
