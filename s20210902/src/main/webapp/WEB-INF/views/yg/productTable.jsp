...
<script type="text/javascript">
	...
	function checkDelete(p_code) {
		if($('#delList').is(':checked') == true){
			var p_code = $('input:checkbox[id="delList"]').val();
			var p_img = $('input:checkbox[id="delList"]').next().val();
			var p_size = $('input:checkbox[id="delList"]').prev().val();
			var result;
	    	result = confirm("삭제 하시겠습니까?");
	    	if(result == true){
		    	$.ajax({
		    		url : "productDelete",
		    		data : {p_code:p_code, p_size:p_size, p_img:p_img},
		    		dataType :'text',
		...
		<div class="container">
			<div class="row">
					...
					<div class="shopping__cart__table">
						...
						<table>
							...
							<c:forEach var="product" items="${tableProduct}">
								<tr>
									<td><input type="hidden" value="${product.p_size }"name="p_size" id="p_size">
										<input name="delList" id="delList" type="checkbox" value="${product.p_code}"/>
										<input type="hidden" value="${product.p_img }"name="p_img" id="p_img">
										<img src="${product.p_img}" alt="" width="100" height="100">
									</td>
									<td>
										<a href="productContent?p_code=${product.p_code} &p_size=${product.p_size}"> ${product.p_code}<br></a>
									</td>
									<td>${product.p_name}<br></td>
									<td>${product.s_name}<br></td>
									<td>${product.p_qty}<br></td>
									<td>${product.cate_name}<br></td>
				...
				<td>${product.cate_name}</td> --%>
						...
						<Br>
						<div align="right">
							<button type="button" onclick="location.href='productWriteForm'" class="site-btnOrder1">등록</button> 
							<button type="button" onclick="javascript:checkDelete(); return false;" class="site-btnOrder1">삭제</button>

						</div>
						<div class="product__pagination">
							<c:if test="${pg.startPage > pg.pageBlock }">
								<a href="productTable?currentPage=${pg.startPage - pg.pageBlock}">&lt;</a>
							</c:if>
							<c:forEach var="i" begin="${pg.startPage }" end="${pg.endPage }">
								<a href="productTable?currentPage=${i }" class="<c:if test="${pg.currentPage == i }">active</c:if>">${i }</a>
							</c:forEach>
