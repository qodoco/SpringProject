    	...
    	<div class="col-lg-12">
    		<div class="product__pagination">       	
				<c:if test="${pg.startPage > pg.pageBlock }">
					<a href="bhProT?currentPage=${pg.startPage-pg.pageBlock}&sort=${sort}">[이전]</a>
				</c:if>
				<c:forEach var="i" begin="${pg.startPage}" end="${pg.endPage}">
					<a href="bhProT?currentPage=${i}&sort=${sort}" class="<c:if test="${pg.currentPage == i }">active</c:if>">${i}</a>
				</c:forEach>
				<c:if test="${pg.endPage < pg.totalPage }">
					<a href="bhProT?currentPage=${pg.startPage+pg.pageBlock}&sort=${sort}">[다음]</a>
				</c:if>
			</div>
		</div>
	</div>			
