    ...
    <div class="container">
		...
		<table class="orderTable2">
			<tr>
				...
				<td width="500px"></td>
			<td align="right">
			<select name='o_cancel' required="required" class="orderSelect">
				<option value='' selected>-주문상태-</option>
				<option value='0'>주문요청</option>
				<option value='1'>주문확정</option>
				<option value='2'>주문취소</option>
				</select>
			<input type="submit" class="site-btnOrder1" value="확인"></td>
			<td>
		<button type="button" onclick="location.href='order1List?currentPage=${pg.currentPage}'" class="site-btnOrder1">최신순</button>
		<button type="button" onclick="location.href='order1List?currentPage=1'" class="site-btnOrder1">목록</button></td>
		</tr>
		</table>
		</form>
		</div>
...
<c:if test="${pg.startPage > pg.pageBlock }">
	<a href="order1Date?currentPage=${pg.startPage-pg.pageBlock}">[이전]</a>
</c:if>
<c:forEach var="i" begin="${pg.startPage}" end="${pg.endPage}">
	<a href="order1Date?currentPage=${i}" class="<c:if test="${pg.currentPage == i }">active</c:if>">${i}</a>
</c:forEach>
<c:if test="${pg.endPage < pg.totalPage }">
	<a href="order1Date?currentPage=${pg.startPage+pg.pageBlock}">[다음]</a>
</c:if>
</div>
