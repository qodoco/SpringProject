    ...
    <section class="breadcrumb-option">
		...
		<table class="orderTable2">
			<tr>
				...
				<td width="600px"></td>
				<td align="right">
				<button type="button" onclick="location.href='order1ListManagerC'" class="site-btnOrder1">완료된 주문내역</button>
				<button type="button" onclick="location.href='order1ListManager?currentPage=1'" class="site-btnOrder1">목록</button>
				</td>
			</tr>
		</table>
	</div>
	...
	<c:set var ="order1Manager" value="${listOrderManager}"></c:set>
		<table class="orderTable">
			<thead class="orderThead">
				...
				<c:if test="${total > 0 }">
				<c:forEach var="order1Manager" items="${listOrderManager }">
				<tbody class="orderTbody">
					<tr>
						<td>
							<div>
								<a href="bhDetail?p_code=${order1Manager.p_code}" style="color: black"><input type="image" src="${order1Manager.p_img }" style="width:100px;height:100px;"></a>
								<ul class="orderLi">
									<li>상품명 : <a href="bhDetail?p_code=${order1Manager.p_code}" style="color: black; font-weight: bold;">${order1Manager.p_name}</a></li>
									<li>사이즈 : ${order1Manager.content}</li>
								</ul>
							</div>
						</td>
						<td width="50px">${order1Manager.o_date }</td>
						<td>${order1Manager.o_num }</td>
						<td>${order1Manager.mem_name }
						<button type="button" class="primary-btnOrder1"  data-target="#exampleModal" onclick="getMemberInfo('${order1Manager.mem_id }')">상세</button>
						</td>
						<td width="80px">${order1Manager.o_accept }</td>
						<td width="120px">${order1Manager.o_tel }</td>
						<td>${order1Manager.o_address }</td>
						<td>
							<div>
								<ul>
									<li>${order1Manager.sumValue }원</li>
									<li style="font-size: 15;">수량 : ${order1Manager.p_qty }개</li>
								</ul>
							</div>
						</td>
						<td>
							<c:choose>
								<c:when test="${order1Manager.o_cancel eq '0'}">
									...
									<a href="order1CancelManager?o_num=${order1Manager.o_num}&p_qty=${order1Manager.p_qty}&p_code=${order1Manager.p_code}&p_size=${order1Manager.p_size}" class="primary-btnOrder1">확정</a>
								</c:when>
							</c:choose>
						</td>
					</tr>
...
<c:forEach var="i" begin="${pg.startPage}" end="${pg.endPage}">
	<a href="order1ListManager?currentPage=${i}" class="<c:if test="${pg.currentPage == i }">active</c:if>">${i }</a>
</c:forEach>
