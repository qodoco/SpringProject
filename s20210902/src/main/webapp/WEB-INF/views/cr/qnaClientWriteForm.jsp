			...
			$.ajax({						/* controller 호출  url, dataType 호출 할 때 정보*/
				...
				success : function(data){
					...
					if(data == 0){
						...
						str +="<table align-content='center'><tr><th>상품</th><th></th><th>주문번호</th></tr>"
						$(data).each(
							function() {
								str += "<tr><td><label for='"+index+"'><input type='radio' class='p_product' name='p_product' id='"+index+"' value1='"+this.p_code+"' value2='"+this.p_size+"' value3='"+this.p_name+"'>&nbsp"
											+this.p_name+"&nbsp</label></td><td><img alt='"+this.p_img+"' src='"+this.p_img+"' width='100px' height='100px'></td><td>"+this.o_num+"</td></tr>";
											index+=1;
							}
						);
						str+="</table>";
			...
			if($('.p_product').is(':checked') == true){
				...
				$("input[name=p_product]:checked").each(
					function(index, item){
				       ...
				       p_name = $(item).attr("value3");
					});
				$('#selectProduct').text(p_name);
...
<body>
    ...
    <div class="container">
		...
		<form action="qnaClientWritePro" method="post" enctype="multipart/form-data">
			<table>
				<tr>
					<th>문의유형</th>
					<td>
						<select name="q_mcode">
		    				<c:forEach var="qna" items="${qMocodeList }">
				...
				<tr>
					<th>문의상품</th>
					<td>
						<input type="hidden" name="p_code" value="0">
						<input type="hidden" name="p_size" value="0">
						<span id="selectProduct"></span>
				...
				<tr>
					<th>내용</th>
					<td>
						<textarea id="InputTextarea" name="q_content" cols="5" rows="1" placeholder="문의내용을 입력해주세요.(1000자 이내)"
