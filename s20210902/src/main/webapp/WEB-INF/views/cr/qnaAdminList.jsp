...
<body>
    ...
    <div class="container">
    	...
    	<ul>
    		<li class="<c:if test='${qna.q_re_status == 0 }'>on</c:if>"><a href="qnaAdminList">미확인</a></li>
    		<li class="<c:if test='${qna.q_re_status == 1 }'>on</c:if>"><a href="qnaAdminList?q_re_status=1">답변대기</a></li>
    		<li class="<c:if test='${qna.q_re_status == 2 }'>on</c:if>"><a href="qnaAdminList?q_re_status=2">답변완료</a></li>
    	</ul>
		<br>
		<br>
    	</div>
		...
		<form action="qnaWritePro" method="post" enctype="multipart/form-data" target="myBatisFrame">
			<table>
				<tr>
					<th>번호</th><th>분류</th><th>아이디</th><th>내용</th><th>등록일</th><th>답변확인</th>
				</tr>
				<c:if test="${total == 0}">
					<tr><td colspan="6">1:1 문의 내역이 없습니다.</td></tr>
				</c:if>
				<c:if test="${total > 0 }">
					<c:forEach var="qna" items="${qnaList }">
						<tr>
							...
							<td>${qna.content }</td>
							<td>${qna.mem_id }</td>
							<td><a href="qnaClientContent?q_num=${qna.q_num }">${qna.q_content }</a></td>
							<td>${qna.q_date }</td>
							<td>
								<c:if test="${qna.q_re_status == 0}">
									미확인
								</c:if>
								<c:if test="${qna.q_re_status == 1}">
									답변대기
								</c:if>
								<c:if test="${qna.q_re_status == 2}">
									답변완료
								</c:if>
							</td>
						</tr>
			...
			<div class="product__pagination">
				<c:if test="${page.startPage > page.pageBlock }">
					<a href="qnaAdminList?&currentPage=${page.startPage - page.pageBlock }">[이전]</a>
				</c:if>
				<c:forEach var="i" begin="${page.startPage }" end="${page.endPage }">
					<a href="qnaAdminList?&currentPage=${i }" class="<c:if test="${page.currentPage == i }">active</c:if>">${i }</a>
				</c:forEach>
				<c:if test="${page.endPage < page.totalPage }">
					<a href="qnaAdminList?currentPage=${page.startPage + page.pageBlock }">[다음]</a>
				</c:if>
