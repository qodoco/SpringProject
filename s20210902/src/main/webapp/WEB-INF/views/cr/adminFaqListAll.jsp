...
<body>
    ...
    <section>
    	...
    	<div class="customer_service_category">
	    	<ul>
	    		<li><a href="adminFaqListAll">전체</a></li>
	    		<li><a href="adminFaqListAll?q_mcode=310">상품</a></li>
	    		<li><a href="adminFaqListAll?q_mcode=320">배송</a></li>
	    		<li><a href="adminFaqListAll?q_mcode=330">교환/환불</a></li>
	    		<li><a href="adminFaqListAll?q_mcode=340">주문결제</a></li>
	    		<li><a href="adminFaqListAll?q_mcode=350">기타</a></li>
	    	</ul>
    	</div>
    	</div>
    	<div class="col-lg-12 col-md-6">
    	<div class="shopping__cart__table">
    		<ul style="list-style: none;">
    			<c:if test="${total == 0 }">
    				<li style="border-top: 1px solid #5555;"><div style="padding-top: 10px;">내역이 없습니다.</div></li>
    			</c:if>
    			<c:if test="${total > 0 }">
	    			<c:forEach var="faq" items="${faqList }">
	    			<c:set var="cnt" value="${cnt+1 }" />
	    			<li id="faqListId${cnt}" class="faqHide" style="border-top: 1px solid #5555;">
	    				<input type="hidden" value="${faq.f_num }">
	    				<div class="title" style="padding: 30px;">
								<span class="cate" style="padding-right: 15%; font-weight: bold;">${faq.content }</span>
								<span class="title"><a href="faqContent?f_num=${faq.f_num }&currentPage=${page.currentPage}">${faq.f_title }</a></span>
						</div>
	    			</li>
	    			</c:forEach>
    			</c:if>
    	...
    	<div>
    		...
    		<button type="button" onclick="location.href='faqWriteForm'" class="primary-btn">글작성</button>
    	</div>
    	<div class="product__pagination">
    		<c:if test="${page.startPage > page.pageBlock }">
				<a href="adminFaqListAll?q_mcode=${faq.q_mcode }&currentPage=${page.startPage - page.pageBlock }">[이전]</a>
			</c:if>
			<c:forEach var="i" begin="${page.startPage }" end="${page.endPage }">
				<a href="adminFaqListAll?q_mcode=${faq.q_mcode }&currentPage=${i }" class="<c:if test="${page.currentPage == i }">active</c:if>">${i }</a>
			</c:forEach>
			<c:if test="${page.endPage < page.totalPage }">
				<a href="adminFaqListAll?q_mcode=${faq.q_mcode }&currentPage=${page.startPage + page.pageBlock }">[다음]</a>
			</c:if>
    	</div>
