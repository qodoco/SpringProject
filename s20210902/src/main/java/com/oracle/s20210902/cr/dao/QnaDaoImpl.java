...
public class QnaDaoImpl implements QnaDao {
	...
	public Qna qnaClientContent(Qna qna) {
		...
		try {
			...
			System.out.println("QnaDaoImpl qnaClientContent-->"+db_p_code);
			if(db_p_code == null || db_p_code.equals("0")) {
	...
	public int qnaClientWrite(Qna qna) {
		...
		try {
			if(qna.getP_code() == 0) {
				result = session.insert("crQnaClientInsertNull", qna);
			}else {
	...
	@Override
	public String qnaClientDelete(Qna qna) {
		int result = 0;
		String result2 = null;
		System.out.println("QnaDaoImpl qnaReplyStatusCheck Start...");
		try {
			result = session.delete("crQnaClientDelete", qna);
			if(result > 0) {
				result2 = "0";
			}
		}catch (Exception e) {
			System.out.println("QnaDaoImpl qnaClientDelete Exception -->"+e.getMessage());
		}
		return result2;
	}
