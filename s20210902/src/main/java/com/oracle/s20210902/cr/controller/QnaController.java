...
//		int adminCheck = qs.adminCheck(adminId);
	...
	@ResponseBody
	public List<Order1> modalSelect(Model model, HttpServletRequest request) {
		System.out.println("QnaController qnaCleintWritePro Start...");
		Order1 order1 = new Order1();
		order1.setMem_id((String)request.getSession().getAttribute("mem_id"));
	...
	@RequestMapping(value = "qnaClientDelete")
	@ResponseBody
	public String qnaClientDelete(Qna qna, HttpServletRequest request, Model model) {
		System.out.println("QnaController qnaClientDelete Start...");
		String uploadPath = request.getSession().getServletContext().getRealPath("/");
		String deleteFileName = uploadPath + qna.getQ_image();
		System.out.println("QnaController qnaClientDelete deleteFileName -->"+deleteFileName);
		int delResult = upFileDelete(deleteFileName);
		System.out.println("QnaController qnaClientDelete delResult-->"+delResult);	
		String result = qs.qnaClientDelete(qna);
		System.out.println("QnaController qnaClientDelete result-->"+result);
		return result;
	}
}
