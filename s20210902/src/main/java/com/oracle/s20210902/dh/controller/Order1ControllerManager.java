...
private static final Logger logger = LoggerFactory.getLogger(Order1Controller.class);
	...
	public String order1ListManager(Model model, String currentPage, Order1 order1, HttpServletRequest request) {
		...
		int mem_admin = (int) request.getSession().getAttribute("mem_admin");
		System.out.println("order1ListManager mem_id ->>>>>>>>>>>> " + mem_id);
		System.out.println("order1ListManager mem_id ->>>>>>>>>>>> " + mem_admin);
		order1.setMem_id(mem_id);
		order1.setMem_admin(mem_admin);
	...
	public String order1Cancel(Model model, Order1 order1) {
		...
		return "dh/order1CancelManagerPro";
	}
	...
	public String order1DateManagerC(Model model, String currentPage, Order1 order1, HttpServletRequest request) {
		...
		int total = os.totalDateC(order1);
		Order1Paging pg = new Order1Paging(total, currentPage);		// 총 데이터 개수, 현재페이지 가져옴
		System.out.println("Order1Controller pg->"+pg);
		order1.setStart(pg.getStart());								// DB상에서 시작 rn값
		order1.setEnd(pg.getEnd());									// DB상에서 끝 rn값
